...
const ProfileLeftSide = ({ profile, onReload }: any) => {
	...
	const updateConnectStatus = (is_follower: boolean = false, is_friend: boolean = false) => {
	...
	return (
		<>
			...
			{!isSelfUser && (
				<div className="flex gap-[10px] flex-wrap justify-end xl:justify-between items-center p-3 pt-14 pb-3">
					<Button
						className={`capitalize ${
							connectStatus === 'following'
								? 'text-black bg-white'
								: 'text-white bg-primary'
						} pt-[14px] pb-[15px] pl-[21px] pr-[20px]`}
						...
						value={
							isConnecting ? (
								...
								<>
									...
									<span className="pl-[12px]">{connectStatus}</span>
								</>
							)
						}
					/>
					<Button
						className="capitalize h-[56px] text-[#333333] bg-[#EEEEEE] text-base font-proxima-nova rounded"
						value={
							<>
								<TiMessages />
								<span className="pl-[11px]">Send Message</span>
							</>
						}
						onClick={goToMessages}
					/>
					<BsThreeDotsVertical className="text-[#999999] text-lg" />
				</div>
			)}

			<div className="px-2 md:px-7 pt-4 md:pt-[34px] pb-3">
				<Label
					value={
						<>

...
import { getTextExcerpt, timeFromNowFns } from 'utils/utils';
...
const NewsFeedSingleItem = (props: INewsFeedSingleItem) => {
	const { project, type, user, text, objectType } = props;
	...
	const goToProjectDetails = (id: any) => {
		router.push(`/user/dashboard/${NEWSFEED_OBJECT_TYPE[objectType]}/${id}`);
	};
	...
	const onClickProjectLike = async () => {
		...
		const apiData: ApiDataType = {
			method: 'post',
			url: `/${NEWSFEED_OBJECT_TYPE[objectType]}/${project?.id}/like/`,
			token: true,
		};
...
export default NewsFeedSingleItem;

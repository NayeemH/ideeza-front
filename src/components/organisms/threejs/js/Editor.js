...
var _POINT_LIGHT = new THREE.PointLight(0xffffff, 1, 0);
_POINT_LIGHT.name = 'DefaultPointLight';

let setSelectedObject;
function Editor(setSelectedObjects) {
  ...
  this.signals = {
    ...
    freezeCamera: new Signal(),
    unFreezeCamera: new Signal(),
    reRender: new Signal(),
    setCameraByAngle: new Signal(),

    getsnapshot: new Signal(),
    transparentBackground: new Signal(),
  };
  ...
  this.pointLight = _POINT_LIGHT.clone();
...
Editor.prototype = {
  setScene: function (scene) {
    ...
    let texts = scene.getObjectByName('Texts');
    if (texts)
      texts.traverse((obj) => {
        if (obj.name === 'Text' && obj?.userData?.value) {
          const loader = new FontLoader();
          loader.load('/font/helvetiker_regular.typeface.json', (font) => {
            var textShapes = font.generateShapes(obj?.userData?.value, 0.25);
  ...
  addObject: function (object, parent, index) {
    ...
    if (object.name === 'DefaultPointLight') this.removeObject(object);
  },
  ...
  clear: function (all) {
    this.history.clear();
    this.storage.clear(this.editorFile, all);
  ...
  fromJSON: async function (json) {
    ...
    if (json?.history) this.history.fromJSON(json.history);

    if (json?.scripts) this.scripts = json.scripts;

    if (json?.scene) {
      if (json?.scene?.geometries) {
        json.scene.geometries.forEach((el, i) => {
          if (el.type === 'EdgesGeometry') {
            let geometry = { ...el.geometry, uuid: el.uuid };
            delete geometry.metadata;
            json.scene.geometries[i] = geometry;
            // json.scene.geometries[i] = { ...el.geometry, uuid: el.uuid };
          }
        });
      }

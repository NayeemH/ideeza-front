...
function SidebarProjectVideo(editor) {
  ...
  resolutionRow.add(
    new UIText(strings.getKey('sidebar/project/resolution')).setWidth('90px')
  );
  ...
  resolutionRow.add(
    new UIText('×').setTextAlign('center').setFontSize('12px').setWidth('12px')
  );
  ...
  videoDurationRow.add(
    new UIText(strings.getKey('sidebar/project/duration')).setWidth('90px')
  );
  ...
  const renderButton = new UIButton(
    strings.getKey('sidebar/project/render')
  ...
  renderButton.onClick(async () => {
    ...
    for (let i = 0; i < frames; i++) {
      ...
      ffmpeg.FS(
        'writeFile',
        `tmp.${num}.png`,
        await fetchFile(canvas.toDataURL())
      );
      currentTime += 1 / fps;

import React, { useState } from 'react';
import Label from '@atoms/label';
import AvatarAtom from '@atoms/avatar';
import Button from '@atoms/button';
import { AiFillEye } from 'react-icons/ai';
// import { BsFillHeartFill } from "react-icons/bs";
import { AUTHOR_AVATAR_PLACEHOLDER } from 'enums/blog';
import OnHoverPlayVideo from '@molecules/on-hover-play';
import { Box } from '@mui/material';
import Tooltip from '@mui/material/Tooltip';
import { useSession } from 'next-auth/react';
import { useRouter } from 'next/router';

const TopRatedSingleProject = (props: any) => {
	const { project, onClickPlaceBid } = props;
	const [hideViewLikeCount, setHideViewLikeCount] = useState(false);

	const getDefaultVideo = () => {
		let __video = null;

		const videos =
			project?.project_videos && project?.project_videos?.length > 0
				? project?.project_videos
				: project?.project_videos && project?.project_videos?.length > 0
				? project?.project_videos
				: [];

		if (videos?.length > 0) {
			videos.forEach((video: any) => {
				if (video?.is_default) {
					__video = video?.video;
				}
			});
		}

		return __video;
	};

	const getDefaultImage = () => {
		let __image = '/images/placeholder.jpg';

		const images =
			project?.project_images && project?.project_images?.length > 0
				? project?.project_images
				: project?.project_images && project?.project_images?.length > 0
				? project?.project_images
				: [];

		if (images?.length > 0) {
			images.forEach((image: any) => {
				if (image?.is_default) {
					__image = image?.image;
				}
			});
		}

		return __image;
	};

	const defaultImage = getDefaultImage();
	const defaultVideo = getDefaultVideo();

	const { status } = useSession();
	const router = useRouter();
	const goToProductDetailPage = (id: any) => {
		if (status === 'authenticated') {
			router.push(`/user/dashboard/project/${id}`); //able to get product
		} else {
			router.push(`/project/details/${id}`); //not getting product
		}
	};

	return (
		<div
			className="cursor-pointer min-h-[520px] lg:min-h-[580px] xl:min-h-[640px] 2xl:min-h-[540px] bg-[#FBFBFB] border border-solid border-[#E6E6E6] rounded-[15px]"
			onMouseOver={() => setHideViewLikeCount(true)}
			onMouseOut={() => setHideViewLikeCount(false)}
		>
			<div className="p-4 px-[20px] rounded-xl">
				{project?.name?.length < 20 ? (
					<div
						className="text-[20px] leading-[34px] text-[#333333] font-bold mb-2"
						onClick={() => goToProductDetailPage(project?.id)}
					>
						{project?.name}
					</div>
				) : (
					<Tooltip title={project?.name}>
						<div
							className="text-[20px] leading-[34px] text-[#333333] font-bold mb-2 truncate ... overflow-hidden"
							onClick={() => goToProductDetailPage(project?.id)}
						>
							{project?.name}
						</div>
					</Tooltip>
				)}
				{/* <Tooltip title={project?.name}>
     
          <div className="text-[20px] leading-[34px] text-[#333333] font-bold mb-2 truncate ... overflow-hidden">
            {project?.name}
          </div>
        </Tooltip> */}

				<div className="relative">
					{/* <img src={project?.image || BLOG_PLACEHOLDER_IMAGE} className="w-full rounded-lg" alt="" /> */}
					<div onClick={() => goToProductDetailPage(project?.id)}>
						<div className="2xl:h-[250px] xl:h-[15rem] h-60 w-full">
							{/* TO DO three._d_script shows error in landing */}
							{/*<ThreeJs
              {...{
                editorType: "projectShowCase",
                viewFile: project?.three_d_script,
                //   ? project?.three_d_script
                //   : "",

                toolbar: "none",
                // hideSidePanel: true,
                hideInfo: true,
                containerClass: "rounded-xl w-full ",
              }}
              // {...{
              //   viewFile: project?.three_d_script,
              //   // editorFile: 7,
              //   // editorType: "projectsShowCase",
              //   toolbar: "none",
              //   // hideSidePanel: true,
              //   hideInfo: true,
              //   containerClass: "rounded-xl w-full ",
              // }}
              // {...{
              //   editorFile: 7,
              //   editorType: "projectsShowCase",
              //   toolbar: "none",
              //   hideSidePanel: true,
              //   hideInfo: true,
              //   containerClass: "rounded-xl w-full ",
              // }}
            />*/}

							{defaultVideo ? (
								<Box
									sx={{
										'& video': {
											height: '253px !important',
										},
									}}
								>
									<OnHoverPlayVideo
										hideScaleView={true}
										poster={defaultImage}
										src={defaultVideo} // TODO: Remove comment after testing //src="https://www.learningcontainer.com/wp-content/uploads/2020/05/sample-mp4-file.mp4"
									/>
								</Box>
							) : (
								<img
									src={defaultImage}
									className=" h-full 2xl:h-[253px] rounded-lg"
									style={{ width: '100%' }}
								/>
							)}
						</div>
						<div className=" left-1 bottom-10 text-md text-[#787878]">
							{!hideViewLikeCount && (
								<div className="absolute bottom-[30px] 2xl:bottom-[30px] right-[5%] bg-gradient-to-r from-[#FA00C5] to-[#550F8A] text-white rounded-2xl px-2 py-1">
									<div className="flex 2xl:w-[200px] justify-between">
										<div className="flex items-center px-1 border-r-2 border-white pr-[10px]">
											<AiFillEye className="text-2xl" />
											<span className="ml-1 text-md pl-[8px]">
												{project?.views || '0'}
											</span>
										</div>

										<div className="flex items-center px-1 border-r-2 border-white pr-[10px]">
											<svg
												className="lg:w-6 lg:h-4 w-4 h-4"
												xmlns="http://www.w3.org/2000/svg"
												viewBox="0 0 31.651 26.018"
											>
												<g
													id="Group_1662"
													data-name="Group 1662"
													transform="translate(-175.215 -645.508)"
												>
													<path
														id="Path_128"
														data-name="Path 128"
														d="M168.435,23.832c0-5.828-7.147-10.551-15.962-10.551-6.612,0-12,2.853-14.44,6.333A6.662,6.662,0,0,0,138.6,28c1.925,2.24,2.7,3.59,4.352,7.554,1.19,2.86,3.746,3.727,6.59,3.747,2.341.017,4.518-1.608,5.909-3.246a17.633,17.633,0,0,1,6.57-4.283c4.048-1.335,6.418-4.161,6.418-7.938"
														transform="translate(38.43 632.227)"
														fill="white"
													/>
													<path
														id="Path_129"
														data-name="Path 129"
														d="M160.276,24.2c-.2-.209-2.052-1.277-2.707-1.746a30.7,30.7,0,0,0-4.411-2.363c-.429-.177-.812-.462-1.228-.673-.585-.3-1.177-.578-1.773-.851a.879.879,0,0,0-1.36.828c-.017.468.052,11.1.039,11.861a1.488,1.488,0,0,0,1.753,1.636,3.545,3.545,0,0,0,1.266-.536,19.22,19.22,0,0,0,2.16-1.343c1.043-.719,5.479-4.943,6.136-5.4a.886.886,0,0,0,.125-1.418"
														transform="translate(36.902 631.571)"
														fill="#FF00C7"
													/>
												</g>
											</svg>
											<span className="text-md ml-1 pl-[8px]">0</span>{' '}
											{/*//TODO: bind here contribute point, when api will update*/}
										</div>

										<div className="flex items-center px-1 pl-2">
											{/* <BsFillHeartFill className="text-lg" /> */}
											<svg
												className="lg:w-5 lg:h-5 w-4 h-4"
												xmlns="http://www.w3.org/2000/svg"
												// width="18"
												// height="15"
												viewBox="0 0 18 15"
												fill="none"
											>
												<path
													d="M0.593002 5.27497V13.72C0.593002 13.9998 0.704154 14.2681 0.902005 14.466C1.09986 14.6638 1.3682 14.775 1.648 14.775H4.813V4.21997H1.648C1.3682 4.21997 1.09986 4.33112 0.902005 4.52897C0.704154 4.72682 0.593002 4.99517 0.593002 5.27497H0.593002Z"
													fill="white"
												/>
												<path
													d="M15.525 5.27497H12.409C12.3171 5.275 12.2267 5.25104 12.1469 5.20545C12.0671 5.15986 12.0006 5.09422 11.9539 5.01503C11.9072 4.93583 11.882 4.84583 11.8808 4.75391C11.8796 4.662 11.9024 4.57136 11.947 4.49097L13.047 2.51197C13.1643 2.29987 13.2312 2.06366 13.2427 1.82158C13.2542 1.57949 13.2099 1.33801 13.1133 1.11575C13.0167 0.893489 12.8703 0.696404 12.6854 0.539702C12.5005 0.383001 12.2821 0.270871 12.047 0.211968L11.268 0.0159678C11.176 -0.00743084 11.0793 -0.00546902 10.9883 0.021645C10.8972 0.048759 10.8152 0.100013 10.751 0.169968L6.544 4.84797C6.10848 5.33226 5.86767 5.96065 5.868 6.61197V12.133C5.86906 12.8322 6.14737 13.5025 6.64189 13.9968C7.13641 14.4911 7.80678 14.7692 8.506 14.77H13.754C14.3318 14.769 14.8933 14.5788 15.3527 14.2284C15.8121 13.8781 16.1442 13.3869 16.298 12.83L17.424 7.66197C17.4901 7.37621 17.4908 7.07921 17.4261 6.79313C17.3614 6.50705 17.2329 6.23928 17.0502 6.0098C16.8676 5.78032 16.6354 5.59505 16.3712 5.46781C16.1069 5.34057 15.8173 5.27465 15.524 5.27497H15.525Z"
													fill="white"
												/>
											</svg>
											<span className="text-md ml-1 pl-[8px]">
												{project?.likes || '0'}
											</span>
										</div>
									</div>
								</div>
							)}
						</div>

						<div className="absolute flex mt-0 flex-wrap top-[108%] left-[-15px] sm:left[0px] lg:px-[15px]">
							<div className="w-full flex gap-[20px] items-center">
								<div className="flex gap-[4px] items-center">
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="31.192"
										height="26.563"
										className="lg:h-6 md:h-4 h-3"
										viewBox="0 0 31.192 26.563"
									>
										<g
											id="Group_4571"
											data-name="Group 4571"
											transform="translate(-2391.096 -1078)"
										>
											<path
												id="Path_11662"
												data-name="Path 11662"
												d="M2422.284,1079.468a1.232,1.232,0,0,0-.793-1.178,1.209,1.209,0,0,0-1.352.349,2.563,2.563,0,0,0-.5.763c-1.12,2.635-2.225,5.275-3.335,7.914-.038.09-.082.177-.145.313a12.714,12.714,0,1,0-24.455,6.925,12.3,12.3,0,0,0,6.209,7.4,12.627,12.627,0,0,0,11.808.039,2.333,2.333,0,0,1-.168.245c-.308.338-.605.688-.936,1-.279.267-.607.481-.9.74a2.733,2.733,0,0,0-.327.437,2.349,2.349,0,0,0,.534.144,3.345,3.345,0,0,0,.761-.073,15.384,15.384,0,0,0,6.078-2.72,4.1,4.1,0,0,0,1.534-2.07,2.54,2.54,0,0,0-.01-1.7,2.085,2.085,0,0,1-2.6.585,2.382,2.382,0,0,1-1.059-2.52,2.963,2.963,0,0,0-2.075,2.389.623.623,0,0,1-.213.385,10.149,10.149,0,0,1-7.918,2.217,10.382,10.382,0,1,1,11.759-11.379c.006.061.012.123.017.184.027.307.044.617.044.931a10.4,10.4,0,0,1-.064,1.139h0a4.965,4.965,0,0,1-.524,1.672,6.219,6.219,0,0,0-.288.681,3.193,3.193,0,0,0-.173,1.816,1.937,1.937,0,0,0,3.176,1.249,3.14,3.14,0,0,0,1.154-1.694q.74-2.441,1.481-4.88,1.586-5.221,3.166-10.444A2.5,2.5,0,0,0,2422.284,1079.468Zm-6.636,9.387c-.273.661-.551,1.32-.829,1.979,0-.015,0-.031,0-.046a11.007,11.007,0,1,0-4.33,8.718c-.018.227-.035.447-.051.662a1.174,1.174,0,0,1-.688,1.018,12.039,12.039,0,1,1,5.905-12.588A.488.488,0,0,1,2415.648,1088.854Z"
												transform="translate(0 0)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11663"
												data-name="Path 11663"
												d="M2421.05,1097.175l-5.407-8.822-5.375,8.828c1.772,1.1,3.506,2.18,5.247,3.248a.381.381,0,0,0,.322-.034q2.539-1.544,5.068-3.1A1.735,1.735,0,0,0,2421.05,1097.175Z"
												transform="translate(-11.853 -6.401)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11664"
												data-name="Path 11664"
												d="M2415.869,1117.974c-1.531-.906-3.067-1.8-4.6-2.7-.069-.041-.142-.076-.213-.113l-.031.045,5.088,7.238,5.15-7.252a.6.6,0,0,0-.278.094c-1.547.9-3.1,1.789-4.637,2.694A.4.4,0,0,1,2415.869,1117.974Z"
												transform="translate(-12.32 -22.975)"
												fill="#ff00c7"
											/>
										</g>
									</svg>
									<Label
										value="Main NFT"
										className="text-primary mr-[20px] lg:mr-0 whitespace-nowrap text-[10px] sm:text-[13px] 2xl:text-[16px] lg:text-md"
									/>
								</div>
								<div className="flex gap-[4px] items-center">
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="27.629"
										className="lg:h-6 md:h-4 h-3"
										height="27.617"
										viewBox="0 0 27.629 27.617"
									>
										<g
											id="Group_4518"
											data-name="Group 4518"
											transform="translate(-1067.857 -1549)"
										>
											<path
												id="Path_11633"
												data-name="Path 11633"
												d="M1087.708,1566.3a5.414,5.414,0,0,1,6,.876,4.9,4.9,0,0,1,1.7,3.041,5.4,5.4,0,0,1-4.477,6.321,5.205,5.205,0,0,1-5.186-2.112,5.352,5.352,0,0,1-.59-5.577,7.039,7.039,0,0,1-6.942-.017,1.041,1.041,0,0,0,.025.162c.117.365.247.725.35,1.094a4.046,4.046,0,0,1,.078.558,2.75,2.75,0,0,1,.057.439,5.808,5.808,0,0,1-.832,2.943,5.412,5.412,0,0,1-1.33,1.478,5.554,5.554,0,0,1-3.644,1.1,5.5,5.5,0,0,1-2.487-.809,5.268,5.268,0,0,1-1.785-1.812,5.8,5.8,0,0,1-.581-1.349,5.663,5.663,0,0,1-.166-2.022,4.666,4.666,0,0,1,.413-1.579,5.546,5.546,0,0,1,1.159-1.689,5.555,5.555,0,0,1,3.761-1.571,5.532,5.532,0,0,1,2.237.462.42.42,0,0,0,.163.036,7.579,7.579,0,0,1-.863-3.472,7.456,7.456,0,0,1,.9-3.463,5.406,5.406,0,0,1-6.575-1.491,5.046,5.046,0,0,1-1.187-4.056,5.236,5.236,0,0,1,2.175-3.724,5.164,5.164,0,0,1,3.851-1.015,5.24,5.24,0,0,1,4.108,2.8,5.33,5.33,0,0,1,.1,4.99,7.1,7.1,0,0,1,7.066-.007,5.357,5.357,0,0,1,.761-5.927,5.091,5.091,0,0,1,3.657-1.882,5.4,5.4,0,0,1,5.839,5.957,5.157,5.157,0,0,1-2.75,4.163,5.3,5.3,0,0,1-4.983.167l-.029.03a1.087,1.087,0,0,0,.059.17,6.857,6.857,0,0,1,.75,2.134,7.522,7.522,0,0,1,.059,1.884,5.349,5.349,0,0,1-.468,1.864c-.114.245-.223.493-.333.741C1087.749,1566.175,1087.738,1566.216,1087.708,1566.3Zm-1.517,1.139c.068.377.135.716.185,1.059a.547.547,0,0,1-.04.311,4.965,4.965,0,0,0-.528,1.288,8.176,8.176,0,0,0-.149.9,3.838,3.838,0,0,0,.111,1.153,4.114,4.114,0,0,0,.755,1.665,4.564,4.564,0,0,0,1.6,1.336,4.316,4.316,0,0,0,1.51.429,4.43,4.43,0,0,0,1.976-.258,4.369,4.369,0,0,0,1.909-1.373,3.951,3.951,0,0,0,.659-1.157,3.377,3.377,0,0,0,.281-1.308c0-.046.009-.092.008-.138a3.919,3.919,0,0,0-.579-2.375,4.392,4.392,0,0,0-1.5-1.534,4.256,4.256,0,0,0-1.36-.547,4.8,4.8,0,0,0-1.648-.055,4.353,4.353,0,0,0-1.713.641.694.694,0,0,1-.384.12c-.318-.029-.631-.1-.947-.157A.973.973,0,0,0,1086.191,1567.44Zm-8.474-13.152a9.13,9.13,0,0,0-.151-.916,4.6,4.6,0,0,0-.633-1.422,4.29,4.29,0,0,0-1.531-1.394,4.435,4.435,0,0,0-1.69-.522,4.38,4.38,0,0,0-1.656.151,4.086,4.086,0,0,0-1.447.728,4.3,4.3,0,0,0-1.361,1.736,7.19,7.19,0,0,0-.331,1.1,2.8,2.8,0,0,0-.067.728,6.933,6.933,0,0,0,.109.783,3.9,3.9,0,0,0,.532,1.43,4.39,4.39,0,0,0,1.565,1.553,4.594,4.594,0,0,0,2.054.6,4.357,4.357,0,0,0,1.107-.112,4.266,4.266,0,0,0,1.363-.533.714.714,0,0,1,.49-.148c.292.051.583.1.876.143.158.023.2-.041.16-.2a7.494,7.494,0,0,1-.157-.808.661.661,0,0,1,.074-.393A5.009,5.009,0,0,0,1077.717,1554.288Zm12.452,4.567c.114-.017.295-.05.476-.068a4.065,4.065,0,0,0,1.235-.336,4.37,4.37,0,0,0,1.741-1.4,4.49,4.49,0,0,0,.679-1.368,4.206,4.206,0,0,0,.184-1.313,4.416,4.416,0,0,0-4.52-4.357,4.651,4.651,0,0,0-.985.146,4.351,4.351,0,0,0-2.76,2.117,4.728,4.728,0,0,0-.565,1.987,3.3,3.3,0,0,0,.133,1.18,6.27,6.27,0,0,0,.459,1.166,1.036,1.036,0,0,1,.128.7c-.037.226-.084.452-.127.678-.03.157.012.218.155.2a3.858,3.858,0,0,0,.741-.107.7.7,0,0,1,.626.1A4.719,4.719,0,0,0,1090.168,1558.854Zm-13.009,8.592a.693.693,0,0,0-.141-.009c-.281.045-.565.082-.843.145a.524.524,0,0,1-.425-.058c-.078-.05-.16-.093-.238-.141a3.9,3.9,0,0,0-1.4-.528,4.6,4.6,0,0,0-2.1.107,4.291,4.291,0,0,0-1.965,1.232,7.76,7.76,0,0,0-.6.83,3.474,3.474,0,0,0-.533,1.553,2.536,2.536,0,0,0-.033.924c.06.257.073.531.13.795a4.064,4.064,0,0,0,.809,1.64,4.4,4.4,0,0,0,1.423,1.176,4.311,4.311,0,0,0,1.61.47,4.411,4.411,0,0,0,1.756-.188,4.376,4.376,0,0,0,2.978-5.121,4.481,4.481,0,0,0-.536-1.381.58.58,0,0,1-.074-.336C1077.026,1568.193,1077.1,1567.834,1077.159,1567.447Zm8.61-4.589a4.089,4.089,0,0,0-1.147-2.872,3.984,3.984,0,0,0-2.944-1.255,3.932,3.932,0,0,0-2.813,1.119,4.1,4.1,0,0,0-.1,5.847,4.084,4.084,0,0,0,5.831-.005A4.066,4.066,0,0,0,1085.769,1562.858Zm-7.819-3.513c-.253-.036-.49-.061-.725-.1a.408.408,0,0,0-.449.24,7.518,7.518,0,0,0-.67,1.328,5.916,5.916,0,0,0-.358,2,5.547,5.547,0,0,0,.2,1.5,5.807,5.807,0,0,0,.958,2.01.245.245,0,0,0,.178.088c.193-.012.383-.048.575-.071.1-.012.2-.015.311-.023a5.137,5.137,0,0,1-.016-6.971Zm7.488,6.954c.2.031.419.056.636.1s.336.024.442-.163c.216-.377.441-.75.623-1.144a4.586,4.586,0,0,0,.425-1.839c0-.071.022-.141.029-.213a2.045,2.045,0,0,0,.021-.365c-.027-.284-.06-.567-.113-.847a7.726,7.726,0,0,0-.223-.935,5.1,5.1,0,0,0-.8-1.546.3.3,0,0,0-.216-.1c-.193.006-.386.046-.58.065a2.6,2.6,0,0,1-.264,0A5.165,5.165,0,0,1,1085.438,1566.3Zm-7.152-7.32a5.22,5.22,0,0,1,6.8.015c0-.07,0-.135.011-.2a4.557,4.557,0,0,1,.091-.54c.031-.112.024-.185-.074-.25-.059-.039-.112-.089-.172-.129a5.391,5.391,0,0,0-1.595-.738,3.783,3.783,0,0,0-.924-.191,5.147,5.147,0,0,0-1.08-.032c-.315.06-.644.065-.965.132a5.737,5.737,0,0,0-2.019.864c-.156.1-.291.205-.169.418.014.024,0,.061.008.091C1078.23,1558.61,1078.259,1558.8,1078.286,1558.979Zm0,7.688c-.029.27-.068.536-.081.8a.3.3,0,0,0,.111.231,5.84,5.84,0,0,0,.822.476,6.437,6.437,0,0,0,1.909.533,6.033,6.033,0,0,0,2.041-.13,5.533,5.533,0,0,0,1.922-.862.284.284,0,0,0,.147-.293c-.024-.182-.052-.363-.073-.544a2.011,2.011,0,0,1,0-.232A5.207,5.207,0,0,1,1078.288,1566.667Z"
												transform="translate(0)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11634"
												data-name="Path 11634"
												d="M1083.882,1582.181a1.487,1.487,0,0,1,.336.191c.458.446.9.9,1.356,1.359l.021.022c.2.218.242.209.447,0,.381-.393.772-.777,1.161-1.163a2.118,2.118,0,0,1,.3-.279.491.491,0,0,1,.787.393.522.522,0,0,1-.159.351c-.217.244-.458.467-.688.7-.3.306-.6.618-.9.922a4.807,4.807,0,0,1-.432.392.438.438,0,0,1-.575.014,1.175,1.175,0,0,1-.191-.154c-.574-.571-1.148-1.141-1.715-1.718a1.093,1.093,0,0,1-.242-.348A.519.519,0,0,1,1083.882,1582.181Z"
												transform="translate(-4.145 -8.87)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11635"
												data-name="Path 11635"
												d="M1071.293,1564.978a.634.634,0,0,1-.2.408c-.475.474-.948.951-1.427,1.422-.1.1-.141.167-.012.292.477.463.94.941,1.409,1.413a.579.579,0,0,1,.2.461.465.465,0,0,1-.286.4.412.412,0,0,1-.5-.051c-.433-.413-.852-.843-1.278-1.264-.193-.191-.383-.384-.583-.565a.983.983,0,0,1-.319-.477.18.18,0,0,1,0-.117,1.314,1.314,0,0,1,.193-.326q.944-.959,1.9-1.906a.488.488,0,0,1,.783.051A1.174,1.174,0,0,1,1071.293,1564.978Z"
												transform="translate(-0.117 -4.143)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11636"
												data-name="Path 11636"
												d="M1101.047,1568.913a1.556,1.556,0,0,1,.182-.338c.252-.275.521-.536.785-.8.246-.248.495-.491.735-.744a.156.156,0,0,0-.009-.149c-.111-.127-.236-.242-.355-.363-.385-.385-.768-.771-1.155-1.153a.5.5,0,0,1-.123-.554.457.457,0,0,1,.39-.31.53.53,0,0,1,.446.166c.659.661,1.319,1.321,1.969,1.991a.441.441,0,0,1-.005.627c-.212.229-.438.445-.657.667-.3.3-.591.61-.892.909a6.5,6.5,0,0,1-.509.466.428.428,0,0,1-.527.034.417.417,0,0,1-.236-.43Z"
												transform="translate(-8.872 -4.143)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11637"
												data-name="Path 11637"
												d="M1087.794,1552.431a.571.571,0,0,1-.4-.194c-.479-.479-.959-.955-1.433-1.438-.1-.1-.158-.11-.267,0-.475.487-.966.96-1.448,1.443a.563.563,0,0,1-.581.143.471.471,0,0,1-.318-.42.531.531,0,0,1,.2-.444l1.447-1.447c.138-.138.273-.278.413-.412a.508.508,0,0,1,.773-.046c.031.026.062.053.091.082q.913.913,1.827,1.828a.484.484,0,0,1-.04.8A1.208,1.208,0,0,1,1087.794,1552.431Z"
												transform="translate(-4.139 -0.123)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11638"
												data-name="Path 11638"
												d="M1099.185,1577.3a1.686,1.686,0,0,1-.321,1.146,2.535,2.535,0,0,1-.443.443,4.1,4.1,0,0,1-.4.289.507.507,0,0,0-.215.4.49.49,0,0,1-.49.457.479.479,0,0,1-.475-.479,1.314,1.314,0,0,1,.722-1.276,1.111,1.111,0,0,0,.64-1.083.879.879,0,0,0-.831-.875.828.828,0,0,0-.895.677c-.023.114-.043.23-.062.345a.48.48,0,0,1-.491.34.464.464,0,0,1-.462-.389,1.8,1.8,0,0,1,1.28-1.884,1.849,1.849,0,0,1,2.314,1.1,5.157,5.157,0,0,1,.174.771Z"
												transform="translate(-7.378 -7.036)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11639"
												data-name="Path 11639"
												d="M1097.355,1582.707a.45.45,0,0,1,.495-.479.472.472,0,0,1,.463.5.481.481,0,0,1-.5.487A.441.441,0,0,1,1097.355,1582.707Z"
												transform="translate(-7.885 -8.883)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11640"
												data-name="Path 11640"
												d="M1072.525,1553.7a2.443,2.443,0,0,1-.134-.389.425.425,0,0,1,.284-.478.5.5,0,0,1,.548.09q.478.457.935.935a.508.508,0,0,1,0,.663c-.32.333-.647.659-.979.98a.49.49,0,0,1-.61.01.537.537,0,0,1-.14-.627c.018-.034.038-.068.067-.12a.341.341,0,0,0-.338.16.648.648,0,0,0,.111.945.9.9,0,0,0,.448.133c1.021.011,2.042.02,3.063,0a.649.649,0,0,0,.308-1.247,1.665,1.665,0,0,0-.391-.064c-.184-.01-.369,0-.554,0a.474.474,0,0,1-.44-.322.546.546,0,0,1,.185-.579.415.415,0,0,1,.2-.052c.333-.005.668-.021,1,0a1.5,1.5,0,0,1,1.135.707,1.557,1.557,0,0,1,.265,1.156,1.634,1.634,0,0,1-1.482,1.371c-1.15-.006-2.3-.009-3.448,0a1.483,1.483,0,0,1-1.05-.433,1.563,1.563,0,0,1-.5-1.435,1.627,1.627,0,0,1,1.414-1.375A.477.477,0,0,0,1072.525,1553.7Z"
												transform="translate(-0.838 -1.012)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11641"
												data-name="Path 11641"
												d="M1099.194,1557.629c0,.462,0,.923,0,1.385a.441.441,0,0,1-.47.467q-.931,0-1.862,0a.442.442,0,0,1-.471-.468q0-1.384,0-2.769a.441.441,0,0,1,.448-.461l1.876-.017a.472.472,0,0,1,.485.48C1099.181,1556.706,1099.194,1557.168,1099.194,1557.629Zm-.957.006c0-.24,0-.481,0-.722,0-.122-.037-.18-.167-.175-.179.007-.358,0-.537,0-.109,0-.16.038-.159.152q0,.738,0,1.475c0,.122.052.163.168.159.174,0,.348-.007.522,0,.139.007.178-.055.174-.183C1098.232,1558.106,1098.237,1557.871,1098.237,1557.635Z"
												transform="translate(-7.627 -1.808)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11642"
												data-name="Path 11642"
												d="M1097.774,1554.177a1.406,1.406,0,0,1-1.4-1.418,1.482,1.482,0,0,1,1.476-1.4,1.435,1.435,0,0,1,1.35,1.475A1.39,1.39,0,0,1,1097.774,1554.177Zm.025-1.814a.425.425,0,0,0-.421.42.415.415,0,1,0,.831,0A.436.436,0,0,0,1097.8,1552.363Z"
												transform="translate(-7.623 -0.631)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11643"
												data-name="Path 11643"
												d="M1073.964,1574.746a.446.446,0,0,1,.473.273c.362.643.733,1.281,1.1,1.921.24.418.483.834.711,1.258a.466.466,0,0,1-.018.485c-.224.349-.456.693-.683,1.039-.245.371-.488.746-.736,1.116-.145.217-.292.433-.448.643a.465.465,0,0,1-.746.032,3.692,3.692,0,0,1-.311-.418c-.2-.3-.394-.6-.593-.9-.3-.448-.585-.9-.9-1.339a.666.666,0,0,1-.039-.819c.431-.726.834-1.469,1.253-2.2.167-.293.342-.583.519-.87A.394.394,0,0,1,1073.964,1574.746Zm-1.058,3.329a1.349,1.349,0,0,0,.157.062c.208.053.415.106.626.15a1.123,1.123,0,0,0,.389.046c.266-.043.526-.122.789-.186.116-.028.17-.068.093-.2-.309-.531-.608-1.068-.913-1.6a.6.6,0,0,0-.1-.094Zm.384,1.178.585.885c.008.012.015.026.023.039.078.1.125.041.172-.03.108-.168.218-.335.327-.5.078-.123.156-.246.252-.4a4.159,4.159,0,0,1-.67.111A4.43,4.43,0,0,1,1073.29,1579.253Z"
												transform="translate(-1.014 -6.881)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11644"
												data-name="Path 11644"
												d="M1088.3,1567.035c0,.354-.008.708,0,1.062a.519.519,0,0,1-.281.494c-.6.352-1.2.7-1.81,1.042a.569.569,0,0,1-.575,0c-.588-.333-1.172-.674-1.76-1.008a.6.6,0,0,1-.328-.589c.011-.558-.016-1.116-.021-1.675,0-.174.015-.348.03-.521a.468.468,0,0,1,.272-.352c.577-.342,1.16-.673,1.74-1.012a.665.665,0,0,1,.745.021c.484.3.982.571,1.473.854l.107.061a.682.682,0,0,1,.412.731A8.208,8.208,0,0,0,1088.3,1567.035Zm-1.435-1.055a5.6,5.6,0,0,1-.574-.335.5.5,0,0,0-.681-.039,6.84,6.84,0,0,1-.626.378c.317.18.608.348.9.508a.162.162,0,0,0,.129-.019C1086.295,1566.318,1086.567,1566.157,1086.87,1565.98Zm-2.307.918c-.02.031-.035.044-.035.056,0,.271-.008.543,0,.813a.179.179,0,0,0,.085.114c.163.105.33.206.5.3.094.054.193.1.323.167,0-.334,0-.624,0-.914,0-.032-.052-.072-.089-.094-.178-.109-.359-.215-.54-.319C1084.724,1566.983,1084.647,1566.944,1084.562,1566.9Zm1.873,1.4.047.034c.27-.163.541-.324.807-.492a.167.167,0,0,0,.053-.119c.005-.235,0-.471,0-.706,0-.1-.043-.128-.137-.073-.177.1-.36.193-.534.3-.087.054-.22.13-.227.205C1086.419,1567.733,1086.435,1568.019,1086.435,1568.3Z"
												transform="translate(-4.19 -4.109)"
												fill="#ff00c7"
											/>
										</g>
									</svg>
									<Label
										value="Physical NFT"
										className="text-primary mr-[20px] lg:mr-0 whitespace-nowrap text-[10px] sm:text-[13px] 2xl:text-[16px] lg:text-md"
									/>
								</div>
								<div className="flex gap-[4px] items-center">
									<svg
										xmlns="http://www.w3.org/2000/svg"
										width="27.984"
										height="27.972"
										className="lg:h-6 md:h-4 h-3"
										viewBox="0 0 27.984 27.972"
									>
										<g
											id="Group_4517"
											data-name="Group 4517"
											transform="translate(-1002.165 -958)"
										>
											<path
												id="Path_11628"
												data-name="Path 11628"
												d="M1016.151,985.97c-.515-.021-1.033-.01-1.543-.07-.562-.066-1.118-.189-1.674-.3a10.831,10.831,0,0,1-2.65-.919c-.68-.333-1.33-.73-1.98-1.12-.469-.281-.351-.312-.724.055q-.438.432-.87.871a.594.594,0,0,1-.476.2,6.549,6.549,0,0,0-.754,0,.318.318,0,0,0-.226.138,1.446,1.446,0,0,1-2.71-.34,1.443,1.443,0,0,1,1.748-1.7,1.409,1.409,0,0,1,.957.762.3.3,0,0,0,.1.12,1.2,1.2,0,0,0,.913-.143c.242-.244.48-.492.724-.735.087-.086.1-.143-.005-.237a14.052,14.052,0,0,1-2.568-2.954,13.5,13.5,0,0,1-1.791-4.06,17.488,17.488,0,0,1-.391-2.236,16.985,16.985,0,0,1-.047-2.273,10.6,10.6,0,0,1,.206-1.541,13.435,13.435,0,0,1,1.4-4.063,14.286,14.286,0,0,1,2.562-3.433c.2-.206.422-.4.642-.589.089-.078.087-.132.01-.21-.275-.276-.541-.564-.828-.826a.843.843,0,0,0-1.067.258,1.248,1.248,0,0,1-1.333.588,1.381,1.381,0,0,1-1.155-.908,1.429,1.429,0,0,1,1.334-1.951,1.463,1.463,0,0,1,1.3.781.329.329,0,0,0,.227.138,6.429,6.429,0,0,0,.754,0,.6.6,0,0,1,.476.2c.335.339.677.671,1.008,1.013a.25.25,0,0,0,.388.042,13.059,13.059,0,0,1,2.941-1.581,17.612,17.612,0,0,1,1.893-.577c.527-.135,1.068-.22,1.607-.309a5.317,5.317,0,0,1,.77-.051c.5-.008.992,0,1.488,0a11.132,11.132,0,0,1,2.135.255,13.763,13.763,0,0,1,2.937.936,14.207,14.207,0,0,1,2.354,1.341.232.232,0,0,0,.356-.039c.328-.336.667-.662.994-1a.661.661,0,0,1,.545-.225c.23.014.463,0,.694,0a.258.258,0,0,0,.263-.158,1.383,1.383,0,0,1,1.488-.754,1.309,1.309,0,0,1,1.14.912,1.432,1.432,0,0,1-1.528,1.935,1.33,1.33,0,0,1-1.1-.781.278.278,0,0,0-.261-.163.983.983,0,0,0-1,.41,4.559,4.559,0,0,1-.486.494c-.087.084-.106.142,0,.237a13.826,13.826,0,0,1,2.31,2.577,12.125,12.125,0,0,1,1.549,2.936c.25.728.479,1.467.664,2.215a8.689,8.689,0,0,1,.2,1.485c.049.7.092,1.411.071,2.115a12.949,12.949,0,0,1-.207,1.723,13.367,13.367,0,0,1-1.543,4.321,14.443,14.443,0,0,1-2.31,3.083c-.231.236-.476.459-.722.68-.1.09-.118.152-.013.253.252.244.485.507.744.742a1.063,1.063,0,0,0,.346.148c.029.011.066,0,.1,0a.585.585,0,0,0,.667-.332,1.287,1.287,0,0,1,1.387-.6,1.348,1.348,0,0,1,1.189,1.144,1.389,1.389,0,0,1-.781,1.583,1.418,1.418,0,0,1-1.61-.229,1.344,1.344,0,0,1-.3-.415.259.259,0,0,0-.264-.16c-.244,0-.489-.007-.734,0a.6.6,0,0,1-.477-.2c-.34-.344-.685-.681-1.023-1.026a.231.231,0,0,0-.356-.041,13.594,13.594,0,0,1-2.974,1.6,13.922,13.922,0,0,1-5.109.945Zm6.226-4.658,0-.092a1.673,1.673,0,0,0-.3-.069.964.964,0,0,1-.926-.87,1.02,1.02,0,0,1,.635-1.02.979.979,0,0,1,1.175.4,1.052,1.052,0,0,1,.145.742.257.257,0,0,0,.076.2c.028.025.138-.008.185-.046.219-.18.439-.361.64-.562a11.607,11.607,0,0,0,1.695-2.112,11.013,11.013,0,0,0,1.087-2.307,10.773,10.773,0,0,0,.526-2.41,11.128,11.128,0,0,0,.08-1.422c-.02-.524-.113-1.046-.179-1.569a9.38,9.38,0,0,0-.663-2.417,9.573,9.573,0,0,0-1.17-2.154c-.394-.521-.812-1.025-1.245-1.513a8.2,8.2,0,0,0-.792-.718c-.037-.032-.129-.059-.154-.039a.363.363,0,0,0-.095.178c-.011.041.015.09.019.136a.966.966,0,0,1-.385.889.983.983,0,0,1-1.036.1.97.97,0,0,1,.413-1.835.946.946,0,0,0,.272-.082l-.007-.08a10.947,10.947,0,0,0-6.1-1.885,10.786,10.786,0,0,0-6.343,1.925.609.609,0,0,0,.246.123,2.33,2.33,0,0,1,.367.064.92.92,0,0,1,.616.887.971.971,0,0,1-1.94.075,1.3,1.3,0,0,0,0-.317.262.262,0,0,0-.1-.173c-.037-.021-.129.011-.172.049-.269.234-.547.461-.793.717a10.745,10.745,0,0,0-1.919,2.631,12.451,12.451,0,0,0-.713,1.652,12.263,12.263,0,0,0-.427,1.826c-.1.622-.146,1.255-.187,1.885a4.267,4.267,0,0,0,.07.768c.056.464.088.934.184,1.39a9.016,9.016,0,0,0,.719,2.216,17.858,17.858,0,0,0,1.019,1.831,13.067,13.067,0,0,0,1.164,1.45,9.821,9.821,0,0,0,.874.8c.046.041.152.077.186.053a.3.3,0,0,0,.1-.2,2.92,2.92,0,0,0,0-.317.946.946,0,0,1,1.383-.78.965.965,0,0,1,.511,1.118.951.951,0,0,1-.932.717.978.978,0,0,0-.261.093l.016.072a10.989,10.989,0,0,0,7.234,1.84A10.621,10.621,0,0,0,1022.377,981.312Zm-15.793-18.171c-.05.08-.09.157-.142.225-.371.489-.78.954-1.106,1.47a18.49,18.49,0,0,0-1.055,1.923,10.743,10.743,0,0,0-.786,2.4,11.489,11.489,0,0,0-.291,2.047c-.013.467-.016.936,0,1.4a9.5,9.5,0,0,0,.1,1.2,12.374,12.374,0,0,0,.337,1.562,18.212,18.212,0,0,0,.721,2.029,12.583,12.583,0,0,0,2.063,3.179c.188.218.194.219.393.021.071-.07.138-.143.211-.21a.163.163,0,0,0,.006-.269,12.208,12.208,0,0,1-2.645-4.752,12.531,12.531,0,0,1-.415-2.193,12.1,12.1,0,0,1,.9-5.966,12.73,12.73,0,0,1,1.941-3.143c.327-.4.329-.4-.045-.76C1006.734,963.268,1006.688,963.231,1006.584,963.141Zm21.847,8.767c-.041.572-.04,1.105-.126,1.624-.117.7-.281,1.394-.463,2.081a8.824,8.824,0,0,1-.995,2.324c-.414.68-.9,1.315-1.359,1.97-.088.126-.27.26-.255.368.019.135.2.243.307.37s.2.125.3,0c.265-.336.545-.663.8-1.005a12.487,12.487,0,0,0,1.866-3.608,12.658,12.658,0,0,0,.6-2.956,11.111,11.111,0,0,0-.1-2.9c-.126-.692-.278-1.38-.466-2.058a10.482,10.482,0,0,0-1-2.386,12.221,12.221,0,0,0-1.525-2.188c-.35-.385-.355-.376-.725.017a.176.176,0,0,0,0,.285,12,12,0,0,1,1.621,2.289,12.175,12.175,0,0,1,1.364,4.026C1028.369,970.747,1028.384,971.351,1028.431,971.908ZM1016.167,985v-.028c.329-.02.66-.029.988-.062a12.3,12.3,0,0,0,1.469-.179c.661-.148,1.313-.344,1.957-.554a11.358,11.358,0,0,0,2.755-1.379c.353-.235.349-.243.055-.537a1.151,1.151,0,0,1-.111-.113c-.086-.113-.168-.111-.284-.03a11.718,11.718,0,0,1-2.452,1.281,11.256,11.256,0,0,1-3.478.79,12.3,12.3,0,0,1-4.289-.45,13.493,13.493,0,0,1-3.187-1.434c-.468-.288-.463-.291-.825.118-.093.106-.091.176.034.25.362.217.715.45,1.081.66a11.891,11.891,0,0,0,4.085,1.435C1014.693,984.889,1015.432,984.93,1016.167,985Zm-7.554-23.589c.191.173.342.319.506.45a.192.192,0,0,0,.17-.011c.131-.076.244-.185.378-.253.724-.366,1.438-.755,2.182-1.075a10.443,10.443,0,0,1,2.4-.653,12.355,12.355,0,0,1,2.53-.12,8.934,8.934,0,0,1,1.376.143,18.6,18.6,0,0,1,1.821.459,11.113,11.113,0,0,1,2.932,1.417c.236.157.232.164.426-.031.066-.066.127-.135.2-.2.12-.1.094-.194-.019-.272a5.377,5.377,0,0,0-.564-.367c-.65-.336-1.291-.7-1.967-.974a15.872,15.872,0,0,0-2.017-.628,11.789,11.789,0,0,0-1.516-.251,8.272,8.272,0,0,0-1.323-.079c-.636.028-1.27.12-1.906.177a9.966,9.966,0,0,0-2.3.547,12.159,12.159,0,0,0-2.555,1.209C1009.125,961.058,1008.9,961.223,1008.613,961.415Zm-4.229,22.773a.43.43,0,1,0-.435.438A.489.489,0,0,0,1004.383,984.188Zm0-24.416a.43.43,0,1,0-.859-.007.43.43,0,1,0,.859.007Zm24.412,24.416a.487.487,0,0,0-.43-.448.493.493,0,0,0-.429.429.43.43,0,1,0,.858.019Zm0-24.4a.481.481,0,0,0-.4-.451.475.475,0,0,0-.455.439.491.491,0,0,0,.434.442A.485.485,0,0,0,1028.793,959.784Zm-20.968,2.251c-.124.1-.239.2-.359.293-.1.075-.1.146-.01.228a4.436,4.436,0,0,0,.327.317c.043.034.173.037.192.008.092-.141.341-.158.312-.389,0-.017-.021-.034-.034-.048-.115-.119-.23-.237-.347-.354A.518.518,0,0,0,1007.824,962.036ZM1024.408,981c-.127.138-.255.273-.377.413-.01.011,0,.055.018.072.132.137.267.273.4.4a.092.092,0,0,0,.089,0,1.93,1.93,0,0,0,.374-.366c.022-.035-.141-.194-.223-.292a1.525,1.525,0,0,0-.154-.152A1.035,1.035,0,0,0,1024.408,981Zm0-18.044a.49.49,0,0,0,.105-.048c.132-.129.266-.257.389-.394a.14.14,0,0,0,0-.135,1.333,1.333,0,0,0-.4-.305c-.062-.022-.193.149-.289.235a1.071,1.071,0,0,0-.137.142c-.023.028-.052.089-.04.1C1024.152,962.689,1024.281,962.822,1024.4,962.952Zm-16.575,18.969c.08-.075.143-.132.2-.191.412-.4.3-.312-.032-.641,0,0-.01-.013-.015-.013-.073,0-.168-.021-.217.016a1.728,1.728,0,0,0-.318.32.2.2,0,0,0,0,.19A4.345,4.345,0,0,0,1007.83,981.921Z"
												transform="translate(0 0)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11629"
												data-name="Path 11629"
												d="M1011.329,974.8c0-1.5.006-3-.005-4.5a.621.621,0,0,1,.34-.6l2.126-1.231q1.475-.854,2.95-1.707,1.383-.8,2.771-1.582a.612.612,0,0,1,.627.007c.774.442,1.541.9,2.312,1.344q1.931,1.118,3.863,2.23c.532.306,1.065.612,1.6.912a.66.66,0,0,1,.373.64q-.012,4.493,0,8.988a.584.584,0,0,1-.289.564q-.649.38-1.3.759-1.26.73-2.52,1.46-1.837,1.059-3.675,2.114c-.149.085-.3.171-.45.251a.518.518,0,0,1-.544-.025q-1.918-1.116-3.843-2.22-1.991-1.148-3.983-2.293a.645.645,0,0,1-.36-.625C1011.336,977.793,1011.329,976.3,1011.329,974.8Zm15.958.019c0-1.363,0-2.725,0-4.088a.361.361,0,0,0-.19-.357c-.721-.414-1.44-.834-2.161-1.249-.922-.53-1.847-1.055-2.768-1.586q-1.048-.6-2.087-1.221a.441.441,0,0,0-.524-.018c-.44.279-.894.537-1.346.8-.927.534-1.857,1.063-2.784,1.6-.962.553-1.921,1.111-2.885,1.662a.4.4,0,0,0-.224.4q.008,3.781,0,7.561a6.127,6.127,0,0,0,.007.614.387.387,0,0,0,.142.251c.219.147.456.266.685.4.787.459,1.571.923,2.361,1.379,1.329.765,2.666,1.516,3.986,2.294a.532.532,0,0,0,.625,0c.881-.521,1.769-1.031,2.656-1.543q2.145-1.24,4.292-2.475a.382.382,0,0,0,.213-.381C1027.284,977.508,1027.287,976.165,1027.287,974.823Z"
												transform="translate(-3.645 -2.825)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11630"
												data-name="Path 11630"
												d="M1015.429,977.349v.207q0,1.9,0,3.81c0,.343-.184.547-.484.545a.51.51,0,0,1-.509-.559q0-3.691,0-7.383a.53.53,0,0,1,.391-.549.491.491,0,0,1,.607.373c.211.66.41,1.323.617,1.985.218.7.442,1.4.661,2.094.153.49.3.982.492,1.474,0-.073.009-.147.009-.22,0-1.032,0-2.064,0-3.1.007-1.163.025-2.327.039-3.49a.516.516,0,0,1,.346-.525.464.464,0,0,1,.544.178.7.7,0,0,1,.1.369c.006,1.561,0,3.122,0,4.684,0,1.23,0,2.46,0,3.691,0,.634-.023,1.268-.038,1.9a.441.441,0,0,1-.386.454.479.479,0,0,1-.585-.29c-.177-.539-.341-1.083-.511-1.625-.144-.459-.29-.918-.432-1.378-.185-.6-.366-1.2-.55-1.8-.089-.287-.179-.574-.27-.862Z"
												transform="translate(-4.883 -5.567)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11631"
												data-name="Path 11631"
												d="M1022.273,976.436c0-2,0-4.008,0-6.012a.627.627,0,0,1,.344-.611c.452-.26.891-.544,1.335-.819a.557.557,0,0,1,.6-.037c.575.311,1.155.613,1.726.931a.508.508,0,0,1,.2.672.49.49,0,0,1-.682.21c-.412-.209-.818-.43-1.219-.661a.42.42,0,0,0-.51,0,5.773,5.773,0,0,1-.641.394.334.334,0,0,0-.192.335c0,.761,0,1.521,0,2.282q0,1.22,0,2.441c0,.25.019.268.274.268.694,0,1.389,0,2.083,0a.45.45,0,0,1,.447.3.5.5,0,0,1-.172.569.672.672,0,0,1-.334.088c-.681.008-1.362,0-2.043,0-.246,0-.255.01-.255.26q0,2.729,0,5.457a.455.455,0,0,1-.547.512.436.436,0,0,1-.408-.411c-.005-.079,0-.159,0-.238Z"
												transform="translate(-8.001 -4.333)"
												fill="#ff00c7"
											/>
											<path
												id="Path_11632"
												data-name="Path 11632"
												d="M1032.634,973.933v.3q0,3.769,0,7.538c0,.362-.2.57-.52.559a.518.518,0,0,1-.47-.435,2.206,2.206,0,0,1,0-.238c0-2.652,0-5.3,0-7.955a.424.424,0,0,0-.284-.455,8.247,8.247,0,0,1-.851-.466.493.493,0,0,1-.2-.7.524.524,0,0,1,.731-.166c.311.16.617.329.926.495q.9.482,1.8.966a.508.508,0,0,1,.258.683.494.494,0,0,1-.7.224c-.108-.049-.211-.108-.317-.162Z"
												transform="translate(-11.172 -5.506)"
												fill="#ff00c7"
											/>
										</g>
									</svg>
									<Label
										value="Virtual NFT"
										className="text-primary mr-[20px] lg:mr-0 whitespace-nowrap text-[10px] sm:text-[13px] 2xl:text-[16px] lg:text-md"
									/>
								</div>
							</div>
							<div className="w-full mt-[24px] flex flex-col justify-between items-center">
								<div className="flex justify-between items-center mt-1 md:mt-0"></div>
								<div
									className={`flex justify-between items-center ${
										project?.on_sale ? '' : 'opacity-40 cursor-not-allowed'
									}`}
								></div>
								<div className="w-full flex justify-start items-center  mt-1">
									<AvatarAtom
										src={
											[project?.user?.profile_photo] ||
											AUTHOR_AVATAR_PLACEHOLDER
										}
										variant="circular"
									/>
									<Label
										value={`@${project?.user?.first_name || ''} ${
											project?.user?.last_name || ''
										} / ${project?.is_visible ? 'Public' : 'Private'} project`}
										className="text-[12px] ml-3 font-semibold text-[#333333]"
									/>
								</div>
								<div className="bg-[#E6E6E6] mt-[20px] w-[100%] h-[1px]"></div>
							</div>
						</div>
					</div>

					<div className=" absolute top-[158%] lg:top-[153%] w-full flex flex-row lg:flex-col md:justify-center items-center 2xl:items-start 2xl:flex-row xl:flex-wrap 2xl:flex-nowrap 2xl:justify-between py-[20px] justify-between">
						<Button
							value="Place bid"
							className="lg:text-base sm:-text-[12px] md-text-[12px] text-[10px] shadow-none font-medium capitalize text-white lg:py-[16px] lg:px-[30px] md:py-[10px] md:px-[15px] sm:py-[8px] sm:px-[12px] py-[5px] px-[10px] bg-primary rounded-[5px] 2xl:mb-0 mb-[10px]"
							variant="contained"
							onClick={onClickPlaceBid}
						/>
						<div className="flex">
							<div>
								<div className="lg:text-[16px] md:text-[12px] text-[10px] text-[#787878] ml-3 sm:ml-3 lg:ml-2 xl:ml-0">
									Countdown
								</div>
								<div className="lg:text-[16px] sm:text-[12px] text-[10px] text-[#333333] mt-[5px] ml-3 sm:ml-3 lg:ml-2 xl:ml-0">
									06:20:59
								</div>
							</div>
							<div className="h-[40px] w-[2px] mx-[20px] bg-[#E6E6E6] self-center"></div>
							<div>
								<div className="lg:text-[16px] md:text-[12px] text-[10px] text-[#787878]">
									Min bid
								</div>
								<div className="lg:text-[16px] md:text-[12px] text-[10px] text-[#333333] mt-[5px]">
									0.01 ETH
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	);
};

export default TopRatedSingleProject;

...
const httpRequest = async (apiData: ApiDataType) => {
	const { method, url, data, token = false, context, config, headers } = apiData;
	...
	switch (method) {
		case 'get':
			return useAxios.get(url, { ...defaultConfig, ...config });

		case 'post':
			return useAxios.post(url, data, { ...defaultConfig, ...config });

		case 'put':
			return useAxios.put(url, data, { ...defaultConfig, ...config });

		case 'patch':
			return useAxios.patch(url, data, { ...defaultConfig, ...config });

		case 'delete':
			return useAxios.delete(url, { ...defaultConfig, ...config });
...
export const apiService = async (apiData: ApiDataType, callback: any) => {
	try {
		const data: AxiosResponse<any, any> | undefined = await httpRequest(apiData);

...
(function (root, mod) {
		...
		addType: function (fn, weight) {
			...
			if (compute)
				for (var d = this.disabled; d; d = d.prev)
					if (d.fn == fn || (fn.originNode && d.fn.originNode == fn.originNode))
						compute = null;
			if (compute)
	...
	Arr.prototype = extend(Obj.prototype, {
		...
		toString: function (maxDepth) {
			...
			return (
				'[' +
				(maxDepth > -3 ? toString(this.getProp('<i>'), maxDepth - 1, this) : '?') +
				']'
			);
		},
	});
			...
			return withDisabledComputing(fn, function () {
				...
				for (var v in scope.props) {
					...
					for (var i = 0; i < args.length; ++i)
						if (fn.argNames[i] == v && i < args.length) args[i].propagate(local);
				}
				var argNames =
	...
	var inferExprVisitor = {
		...
		CallExpression: fill(function (node, scope, c, out) {
			...
			if (node.callee.type == 'MemberExpression') {
				...
				if (
					...
					scope.fnType &&
					scope.fnType.args.indexOf(self) > -1
				)
		...
		Identifier: ret(function (node, scope) {
			if (node.name == 'arguments' && scope.fnType && !(node.name in scope.props))
				scope
					.defProp(node.name, scope.fnType.originNode)
	...
	function makePredicate(origins, start, end) {
		...
		if (arr) {
			...
			return function (n, pos) {
				return (
					pos && pos.start >= start && pos.end <= end && origins.indexOf(n.origin) > -1
				);
			};
		} else {
			if (end == null)
	...
	var findTypeFromContext = {
		...
		ReturnStatement: function (_parent, node, get) {
			...
			if (fnNode) {
				var fnType =
					fnNode.node.type == 'FunctionExpression'
	...
	exports.typeFromContext = function (ast, found) {
		...
		if (findTypeFromContext.hasOwnProperty(parent.type)) {
			type = findTypeFromContext[parent.type](
				parent,
				found.node,
				function (node, fromContext) {

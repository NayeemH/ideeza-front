...
(function (root, mod) {
	...
	if (typeof define == 'function' && define.amd)
		// AMD
		return define(
			['exports', './infer', './signal', 'acorn/dist/acorn', 'acorn/dist/walk'],
			mod
		);
	...
	function resolveFile(srv, localFiles, name) {
		...
		var offset;
		file.offset = offset = resolvePos(
			realFile,
			file.offsetLines == null ? file.offset : { line: file.offsetLines, ch: 0 },
			true
		);
		var line = firstLine(file.text);
		var foundPos = findMatchingPosition(line, realFile.text, offset);
		var pos =
			foundPos == null ? Math.max(0, realFile.text.lastIndexOf('\n', offset)) : foundPos;
		...
		infer.withContext(srv.cx, function () {
			...
			var scope = (file.scope =
				scopeDepth(scopeStart) < scopeDepth(scopeEnd) ? scopeEnd : scopeStart);
	...
	function isPosition(val) {
		return (
			typeof val == 'number' ||
			(typeof val == 'object' && typeof val.line == 'number' && typeof val.ch == 'number')
		);
	}
	...
	function findCompletions(srv, query, file) {
		...
		function gather(prop, obj, depth, addInfo) {
			...
			if (
				query.filter !== false &&
				word &&
				(query.caseInsensitive ? prop.toLowerCase() : prop).indexOf(word) !== 0
			)
				return;
		...
		if (query.variable) {
			...
			if (
				...
				(expr.node.type == 'ObjectExpression' ||
					wide ||
					(start == null ? end : start) - expr.node.start < 20 ||
					expr.node.end - end < 20)
			)
				return expr;
			return null;
		}
	...
	function findRefsToVariable(srv, query, file, expr, checkShadowing) {
		...
		if (!scope)
			throw ternError(
				'Could not find a definition for ' + name + ' ' + !!srv.cx.topScope.props.x
			);

	...
	function postParse(ast, text) {
		...
		walk.simple(ast, {
			...
			ObjectExpression: function (node) {
				for (var i = 0; i < node.properties.length; ++i) attachComments(node.properties[i]);
			},
			...
			ast,
			{
				...
				CallExpression: function (node, scope) {
					if (node.commentsBefore && isDefinePropertyCall(node)) {
						var type = infer
							.expressionType({ node: node.arguments[0], state: scope })
							.getObjType();
	...
	function postLoadDef(data) {
		...
		if (defs)
			for (var name in defs)
				cx.parent.jsdocTypedefs[name] = maybeInstance(
	...
	function parseTypeInner(scope, str, pos) {
		...
		if (str.indexOf('function(', pos) == pos) {
			...
			if (/^(number|integer)$/i.test(word)) type = infer.cx().num;
				...
				while (str.charCodeAt(pos) == 46 || acorn.isIdentifierChar(str.charCodeAt(pos)))
					++pos;
			...
			while ((m = decl.exec(comment))) {
				...
				switch (m[1]) {
					...
					case 'argument':
						...
						var argname =
							name[2] +
							(parsed.isOptional || (name[1] === '[' && name[3] === ']') ? '?' : '');
	...
	function applyType(type, self, args, ret, node, aval) {
		...
		if (node.type == 'VariableDeclaration') {
			...
			if (decl.init && decl.init.type == 'FunctionExpression')
				fn = decl.init.body.scope.fnType;

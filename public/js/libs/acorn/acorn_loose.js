				...
				function (require, module, exports) {
					...
					lp.parseMaybeConditional = function (noIn) {
						...
						if (this.eat(tt.question)) {
							...
							node.alternate = this.expect(tt.colon)
					...
					lp.parseExprAtom = function () {
						...
						switch (this.tok.type) {
							...
							case tt.name:
								...
								return this.eat(tt.arrow)
									? this.parseArrowExpression(this.startNodeAt(start), [id])
									: id;
							...
							case tt._false:
								node = this.startNode();
								node.value =
									this.tok.type === tt._null ? null : this.tok.type === tt._true;
							...
							case tt.parenL:
								...
								if (this.eat(tt.arrow)) {
									return this.parseArrowExpression(
										...
										inner.expressions || (isDummy(inner) ? [] : [inner])
									);
								}
							...
							case tt._yield:
								...
								if (
									...
									(this.tok.type != tt.star && !this.tok.type.startsExpr)
								) {
									node.delegate = false;
									node.argument = null;
								} else {
					...
					lp.parseObj = function () {
						...
						while (!this.closes(tt.braceR, indent, line)) {
							...
							if (this.eat(tt.colon)) {
								...
								this.tok.type != tt.comma &&
								this.tok.type != tt.braceR
							) {
					...
					lp.parsePropertyName = function (prop) {
						...
						var key =
							this.tok.type === tt.num || this.tok.type === tt.string
								? this.parseExprAtom()
								: this.parseIdent();
					...
					lp.parseIdent = function () {
						var name =
							this.tok.type === tt.name ? this.tok.value : this.tok.type.keyword;
					...
					lp.parseMethod = function (isGenerator) {
						...
						node.expression =
							this.options.ecmaVersion >= 6 && this.tok.type !== tt.braceL;
				...
				function (require, module, exports) {
						...
						switch (starttype) {
							...
							case tt._continue:
								...
								if (this.semicolon() || this.canInsertSemicolon()) {
									...
									node.label =
										this.tok.type === tt.name ? this.parseIdent() : null;
									this.semicolon();
								}
								return this.finishNode(
									node,
							...
							case tt._for:
								...
								if (
									this.tok.type === tt._var ||
									this.tok.type === tt._let ||
									this.tok.type === tt._const
								) {
									var _init = this.parseVar(true);
									if (
										_init.declarations.length === 1 &&
										(this.tok.type === tt._in || this.isContextual('of'))
									) {
							...
							case tt._switch:
								...
								while (!this.closes(tt.braceR, blockIndent, line, true)) {
									if (
										this.tok.type === tt._case ||
										this.tok.type === tt._default
									) {
					...
					lp.parseVar = function (noIn) {
						...
						do {
							...
							decl.id =
								this.options.ecmaVersion >= 6
									? this.toAssignable(this.parseExprAtom())
									: this.parseIdent();
					...
					lp.parseClass = function (isStatement) {
						...
						while (!this.closes(tt.braceR, indent, line)) {
							...
							if (
								...
								this.tok.type != tt.parenL &&
								this.tok.type != tt.braceL
							) {
								method['static'] = true;
								isGenerator = this.eat(tt.star);
								this.parsePropertyName(method);
						...
						for (;;) {
							try {
								...
								if (/unterminated/i.test(msg)) {
									...
									/invalid (unicode|regexp|number)|expecting unicode|octal literal|is reserved|directly after number/i.test(
										msg
									)
								) {
									while (
								...
								this.resetTo(pos);
								if (replace === true)
									replace = { start: pos, end: pos, type: tt.name, value: '✖' };
								if (replace) {
									if (this.options.locations)
